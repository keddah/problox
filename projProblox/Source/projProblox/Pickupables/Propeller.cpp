...
APropeller::APropeller()
{
	...
	sounder->AddAbilitySFX(TEXT("/Script/MetasoundEngine.MetaSoundSource'/Game/Audio/MetaSounds/MS_propeller.MS_propeller'"));
...
{
	if(sounder) sounder->SetFloatParam("roll", mesh->GetRelativeRotation().Roll);
	Super::Ability(deltaTime);
	sounder->RunAudioFunction("print");
